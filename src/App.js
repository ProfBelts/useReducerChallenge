...
analogy to explain how useReducer works, but it's simplified (we're not using account numbers here)
...
4. When the account is opened, isActive is set to true. There is also a minimum deposit 
amount of 500 to open an account (which means that the balance will start at 500)
...
7. Customer can only close an account if there is no loan, AND if the balance is zero. If this 
condition is not met, just return the state. If the condition is met, the account is deactivated 
and all money is withdrawn. The account basically gets back to the initial state
*/
...
export default function App() {
  ...
  return (
    <div className="App">
      ...
      <p>Balance: {balance ? balance : balance + loan}</p>
      <p>Loan: {loan}</p>
